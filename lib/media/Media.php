...
class Media
{
	...
	public function __construct(Projects_Project $Project)
	{
		...
		if (!is_dir($this->_mediaFolder)) {
			Utils_System_File::mkdir($this->_mediaFolder);
		}
		...
		// Sites
		Utils_System_File::mkdir(CMS_DIR .'media/sites/');
		Utils_System_File::mkdir($this->_path);

		// Cache
		Utils_System_File::mkdir(CMS_DIR .'media/cache/');
		Utils_System_File::mkdir($this->_cache);
	}
	...
	/**
	 * Wirft ein QException wenn nicht erlaubte Zeichen im String vorkommen
	 *
	 ...
	 * @return MF Object || QException
	 */
	public function get($id)
	{
		if (!is_int($id)) {
			throw new QException('ID '. $id .' ist kein Integer');
		}
		...
		if (!isset($result[0])) {
			throw new QException('ID '. $id .' wurde nicht gefunden', 404);
		}
