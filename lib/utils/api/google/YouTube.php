...
class Utils_Api_Google_YouTube extends \QUI\QDOM
{
    /**
     * http client
    ...
    public function _getYouTube()
    {
        ...
        if ($this->getAttribute('username') == false) {
            throw new \QUI\Exception('There was no username');
        }

        if ($this->getAttribute('password') == false) {
            throw new \QUI\Exception('There was no password');
        }

        if ($this->getAttribute('appid') == false) {
            throw new \QUI\Exception('There was no appid');
        }

        if ($this->getAttribute('clientid') == false) {
            throw new \QUI\Exception('There was no clientid');
        }

        if ($this->getAttribute('devkey') == false) {
            throw new \QUI\Exception('There was no devkey');
        }
    ...
    {
        if (!file_exists($file)) {
            throw new \QUI\Exception('File not exist', 404);
        }

        $PT_Info = \QUI\Utils\System\File::getInfo($file);
