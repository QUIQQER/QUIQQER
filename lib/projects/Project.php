...
/**
 * This file contains the Projects_Project
 */
...
class Projects_Project
{
	...
	public function __construct($name, $lang=false, $template=false)
	{
		...
		if (!isset($config[$name])) {
			throw new QException('Project Error; Project "'. $name .'" doesnt exist in Config File', 404);
		}
		...
		// Rechte prüfen
		if ($this->_checkRights() == false) {
			throw new QException('You have no rights to edit this project', 403);
		}

		// Langs
		if (!isset($this->_config['langs'])) {
			throw new QException('Project Error; No Language exist', 400);
		}
		...
		if (!isset($this->_config['default_lang'])) {
			throw new QException('Project Error; No default Language defined', 404);
		}
		...
		if ($lang != false)
		{
			if (!in_array($lang, $this->_langs)) {
				throw new QException('Project Error; Language "'. $lang .'" in Project doesnt exist', 404);
			}
		...
		} else
		{
			...
			if (!isset($this->_config['default_lang'])) {
				throw new QException('Project Error;- No Default Language exist', 404);
			}
        ...
        // tabellen setzen
		$this->_TABLE        = $this->_name .'_'. $this->_lang .'_sites';
		$this->_RELTABLE     = $this->_TABLE .'_relations';
		$this->_RELLANGTABLE = $this->_name .'_multilingual';
	...
	{
		return 'Object Project('. $this->_name .','. $this->_lang .')';
	}
	 ...
	 *
	 * @return Projects_Media
	 */
	public function getMedia()
	{
		return new Projects_Media($this);
	}
	...
	public function setup()
	{
		...
		foreach ($this->_langs as $lang)
		{
			$table  = $this->_name .'_'. $lang .'_sites';
