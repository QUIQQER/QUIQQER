...
class QUI_Rewrite
{
	...
	public function getProject()
	{
		...
		{
		    if (isset($vhosts[$_SERVER['HTTP_HOST']]['project']))
			{
				$pname = $vhosts[$_SERVER['HTTP_HOST']]['project'];

				if (isset($config[$pname]))
				{
					//$lang = false;
					if (isset($vhosts[$_SERVER['HTTP_HOST']]['lang']) && !$this->_lang) {
						$this->_lang = $vhosts[$_SERVER['HTTP_HOST']]['lang'];
					}
					...
					try
					{
						...
						$Project = QUI::getProject($pname, $this->_lang, $template);
					} catch(Exception $e)
					{
						// nothing todo
						$Project = false;
		...
		/**
		 * Wenn kein VHOST gefunden wurde
		 */
		...
		//throw new QException('Kein Projekt gefunden');
	}
