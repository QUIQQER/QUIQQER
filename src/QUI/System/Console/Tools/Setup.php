...
class Setup extends QUI\System\Console\Tool
{
    ...
    public function execute(): void
    {
        ...
        $quiqqer = QUI::getPackageManager()->getPackage('quiqqer/core');
        $reference = '';
        ...
        $data = QUI::getLocale()->get('quiqqer/core', 'console.tool.setup.start.data', [
            'version' => QUI::version(),
        ...
        $data = array_map(static fn($entry) => trim($entry), $data);
        ...
        $this->writeLn(QUI::getLocale()->get('quiqqer/core', 'console.tool.setup.start.message'));
        QUI\Setup::all($this);

        $this->writeLn(QUI::getLocale()->get('quiqqer/core', 'console.tool.setup.message.success'));
        $this->writeLn();
    }
}
