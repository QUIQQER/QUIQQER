...
class Manager
{
    ...
    public function upload(): bool | string | array
    {
        ...
        if ($filename === null && isset($_REQUEST['uploadid'])) {
            $legacyUploadId = $this->validateLegacyUploadId($_REQUEST['uploadid']);
        }
        ...
        if ($filename === null) {
            ...
            $this->flushAction(
                'UploadManager.isFinish(' . json_encode($legacyUploadId) . ')'
            );
    ...
    /**
     * Validate an identifier generated by String.uniqueID() in the legacy upload client.
     *
     * @throws Exception
     */
    protected function validateLegacyUploadId(mixed $uploadId): string
    {
        if (!is_string($uploadId) || !preg_match('/\A[a-z0-9]+\z/D', $uploadId)) {
            throw new Exception('Invalid legacy upload identifier.', 400);
        }
