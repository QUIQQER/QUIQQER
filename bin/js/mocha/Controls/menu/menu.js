...
define('mocha/Controls/menu/menu', function()
{
    MUI.Menu = new NamedClass('MUI.Menu', {
        ...
        _buildItems:function(ul, items, addArrow)
        {
            for (var i = 0; i < items.length; i++)
            {
                ...
                if (i > 0 && items[i - 1].type == 'divider') {
                    li.addClass('divider');
                }

                var text = item.text;
                    text = text.substring(0, 19) + (text.length > 19 ? '...' : '');

                var a = new Element('a', {
                    'class' : 'toolMenuLink',
                    html    : '<span class="toolMenuLinkText">'+ text +'</span>',
                    alt     : item.text,
                    title   : item.text
                }).inject( li );
                ...
                if (item.click)
                {
                    ...
                    {
                        QUI.lib.Menu.click( this );
                        event.stop();
                    }.bind(item));
                } else if (!url || item.registered)
                {
                    url = '#';
                ...
                {
                     a.addEvent('click', MUI.sendContentToPartner(this, url, partner, partnerMethod));
                } else
                {
        ...
        {
            if (!e.target) e = new Event(e).stop();
