...
define('mocha/Controls/desktop/desktop', [

    'mocha/Controls/dock/dock',
    'mocha/Controls/taskbar/taskbar'
...
], function()
{
    ...
    MUI.Desktop = new NamedClass('MUI.Desktop', {
        ...
        draw: function(container)
        {
            ...
            var func_replace = function (_, i, r) {
            ...
            for (var idx = 0; idx < this.options.content.length; idx++)
            {
                ...
                if (!section.id) {
                    section.id = this.id + section.name.replace(/(\w)(\w*)/g, func_replace);
                }
                ...
                if (section.name == 'content')
                {
                    ...
                    if (section.columns)
                    {
                        for (var i = 0; i < section.columns.length; i++)
                        {
                            ...
                            column.element = new Element('div', {
                                id : column.id
                            }).inject(this.el.content);
                ...
                } else
                {
                    ...
                    {
                        this.el[section.name] = new Element('div', {
                    ...
                    } else
                    {
                        ...
                        var e = section.element = this.el[section.name] = new Element('div', {
                            id : section.id
                        }).inject( this.el.element );

                        if (section.cssClass) {
                            e.addClass(section.cssClass);
                        }
            ...
            var addToContainer = function()
            {
                ...
                // resize the desktop
                this.setDesktopSize.delay(100, this);
        ...
        setDesktopSize: function()
        {
            ...
            if (this.el.element) {
                this.el.element.setStyle('height', dimensions.height);
            }
            ...
            setTimeout(function()
            {
                MUI.each(function(instance)
                {
                    ...
                    if (instance.isMaximized)
                    {
                        ...
                        {
                            resizeDimensions = $(options.container).getCoordinates();
                        } else
                        {
                            resizeDimensions = document.getCoordinates();
                        }
    ...
    MUI.Window.implement({
        ...
        maximize: function()
        {
            ...
            if (options.container != MUI.desktop.el.content)
            {
                ...
                if (options.restrict) {
                    windowDrag.container = this.el.desktop;
                }
            }
            ...
            if (this.el.maximizeButton) {
                this.el.maximizeButton.setProperty('title', 'Restore');
            }
        ...
        _restoreMaximized: function()
        {
            ...
            if (this.el.maximizeButton) {
                this.el.maximizeButton.setProperty('title', 'Maximize');
            }
    ...
    MUI.append({
        ...
        panelHeight2 : function(column, changing, action)
        {
            ...
            var parent = column.getParent(),
                ...
                columnHeight   = parent.getStyle('height').toInt(),
                panelsExpanded = [],
                panels         = [];
            ...
            panels.each(function(panel)
            {
                ...
                if (panel.getParent().hasClass('expanded') &&
                    panel.getParent().getNext('.expanded'))
                {
                    instance.partner = panel.getParent()
                                            .getNext('.expanded')
                                            .getElement('.panel');
                    ...
                    if (el.hasClass('panel'))
                    {
                        ...
                        } else if (action === null || action == 'collapsing')
                        {
                            ...
                            if ( !instance.isCollapsed &&
                                 (!anyNextContainsChanging(el) || !anyNextSiblingsExpanded(el)))
                            {
                                panelsToResize.push(el);
                                this.panelsTotalHeight += el.offsetHeight.toInt();
                            }
                        } else if (action == 'expanding' && !instance.isCollapsed && el != changing)
                        {
                            ...
                            if (!anyNextContainsChanging(el) ||
                               (!anyExpandingNextSiblingsExpanded(el) && nextExpandedChanging(el)))
                            {
                                panelsToResize.push(el);
                                this.panelsTotalHeight += el.offsetHeight.toInt();
                            }
            ...
            parent.getChildren('.columnHandle').each(function(handle)
            {
                ...
                // Keeps IE7 and 8 from throwing an error when collapsing a panel within a panel
                if (parent.getStyle('height').toInt() < 1) {
                    return;
                }

                var handleHeight = parent.getStyle('height').toInt() -
        ...
        { // May rename this resizeIframeEl()
            var instance       = MUI.get(panel.id),
                contentWrapper = instance.el.contentWrapper;
                ...
                if ( !Browser.ie )
                {
                    instance.el.iframe.setStyles({
                        ...
                        width  : contentWrapper.offsetWidth -
                                 contentWrapper.getStyle('border-left').toInt() -
                                 contentWrapper.getStyle('border-right').toInt()
                    });
                } else
                {
                    instance.el.iframe.setStyles({
    ...
    MUI.Windows = Object.append((MUI.Windows || {}),
    {
        ...
        {
         // Use a negative number if neccessary to place first window where you want it
