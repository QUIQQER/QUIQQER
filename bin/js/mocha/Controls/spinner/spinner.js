...
MUI.Spinner = new NamedClass('MUI.Spinner', {
    ...
    draw: function(container)
    {
        ...
        this.el.spinnerWrapper = new Element('div', {
            'id'    : o.id + 'spinnerWrapper',
            'class' : 'spinnerWrapper'
        }).inject( div );

        this.el.spinner = new Element('div', {
            'id'    : o.id + '_spinner',
            'class' : 'spinner'
        }).inject( this.el.spinnerWrapper );
    ...
    hide: function()
    {
        ...
        if ( this.el.element ) {
            this.el.removeClass('spinnerOverlay');
        }
    ...
    show: function()
    {
        ...
        if ( this.el.element ) {
            container = this.el.element.getParent();
        }

        if ( container && !this.options.inDock )
        {
            var sz = container.getScrollSize();
            var x = parseInt((sz.x / 2) - 16, 10);
            var y = parseInt((sz.y / 2) - 16, 10);

            this.el.element.addClass('spinnerOverlay').setStyles({
                width  : sz.x,
                height : sz.y
            });

            this.el.spinnerWrapper.setStyles({
                position : 'absolute',
                left     : x,
                top      : y,
                zIndex   : 1000
