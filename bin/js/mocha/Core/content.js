...
MUI.Content = Object.append((MUI.Content || {}), {
    ...
    update: function(content){
        ...
        if ( !content.loadMethod )
        {
            if (typeof instance === 'undefined' ||
                typeof instance.options === 'undefined' ||
                !instance.options.loadMethod)
            {
    ...
    gotoPage: function(content, page){
        ...
        var lastPage = parseInt(content.paging.total / content.paging.pageSize, 10);
    ...
    setPageSize: function(content, max){
        ...
        paging.pageSize = max;
        paging.page = 1;
        paging.pageMax = parseInt(paging.total / paging.pageSize, 10);
        if (content.instance && content.instance.updateStart) content.instance.updateStart(content);
        MUI.Content.Providers[content.loadMethod].doRequest(content);
        return this;
    },
...
MUI.Content.Providers.xhr = {
    ...
    doRequest: function(content){
        ...
        if (content.url === null && content.require.js && content.require.js.length !== 0)
        {
            if (Browser.ie6)
            {
                content.fireLoaded.delay(50, content);
            } else
            {
                content.fireLoaded();
            }
...
MUI.Content.Providers.json = {
    ...
    _onSuccess: function(json)
    {
        ...
        if (json !== null)
        {    // when multiple results are persisted, null is returned.  decoding takes place in persistStore instead, and filtering is not allowed
    ...
    doRequest: function(content){
        if (content.content && !content.url){
            if (Browser.ie6) {
                content.fireLoaded.delay(50, this);
            } else {
                content.fireLoaded();
            }
            return;
