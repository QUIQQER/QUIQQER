        ...
        if (shadows){
            for (var x = 0; x <= shadowBlur; x++){
                this.roundedRect(
                    ...
                    [0, 0, 0],
                    x == shadowBlur ? 0.29 : 0.065 + (x * 0.01)
                );
            }
        }
    ...
    drawGauge: function(ctx, width, height, shadowBlur, shadowOffset, shadows, canvasHeader, headerHeight, bodyBgColor, useCSS3){
        if (shadows && !useCSS3){
            if (Browser.webkit){
                ...
                MUI.Canvas.circle(
                    ctx,
                    width * 0.5 + shadowOffset.x,
                    (height + headerHeight) * 0.5 + shadowOffset.x,
                    (width * 0.5) - (x * 2) - shadowOffset.x,
                    [0, 0, 0],
                    x == shadowBlur ? 0.75 : 0.075 + (x * 0.04)
                );
            }
        }
        MUI.Canvas.circle(
            ctx,
            width * 0.5 - shadowOffset.x,
            (height + headerHeight) * 0.5 - shadowOffset.y,
            (width * 0.5) - shadowBlur,
            bodyBgColor,
            1
        );
